...
use crate::state::{CoreEvt, CoreHandleResult, InitResult, ParentState, StateFn};
...
impl<UserStateMachine: ProtoStateMachine> StateMachine<UserStateMachine> {
    ...
    fn dispatch_get_super_state(
        user_state_machine: &mut UserStateMachine,
        state_fn: StateFn<UserStateMachine>,
    ) -> ParentState<UserStateMachine> {
        let get_parent_state_evt =
            CoreEvt::<<UserStateMachine as ProtoStateMachine>::Evt>::GetParentStateEvt;
    ...
    fn reach_init_target(&mut self, target_state_fn: StateFn<UserStateMachine>) {
        ...
        while let InitResult::TargetState(next_target_state) =
    ...
    fn handle_ignored_evt(
        &mut self,
        parent_state_variant: ParentState<UserStateMachine>,
        evt: &CoreEvt<<UserStateMachine as ProtoStateMachine>::Evt>,
    ) {
    ...
    fn find_dissociate_states<'a>(
        &mut self,
        original_state_link: &'a Link<'a, UserStateMachine>,
        target_state_link: &'a Link<'a, UserStateMachine>,
    ) -> (
        Option<&'a Link<'a, UserStateMachine>>,
        Option<&'a Link<'a, UserStateMachine>>,
    ) {
    ...
    fn enter_substates(
        user_state_machine: &mut UserStateMachine,
        target_state_link: &Link<UserStateMachine>,
    ) {
        let mut target_state_link = target_state_link;
    ...
    fn reach_target_state(
        &mut self,
        original_state_link: Link<UserStateMachine>,
        target_state_link: Link<UserStateMachine>,
    ) {
        if let ParentState::Exists(original_state_parent_fn) = Self::dispatch_get_super_state(
            &mut self.user_state_machine,
            ...
            if let ParentState::Exists(target_state_parent_fn) = Self::dispatch_get_super_state(
                &mut self.user_state_machine,
            ...
            if let ParentState::Exists(parent_state_fn) = Self::dispatch_get_super_state(
                &mut self.user_state_machine,
                target_state_link.state_fn,
            ) {
                let new_target_state_link = Link {
    ...
    fn handle_transition(
        &mut self,
        handling_state_fn: StateFn<UserStateMachine>,
        target_state_fn: StateFn<UserStateMachine>,
    ) {
        Self::exit_substates(
            &mut self.user_state_machine,
            self.curr_state,
    ...
    fn dispatch_core_event(
        ...
        match core_handle_result {
            ...
            CoreHandleResult::Ignored(parent_state_fn) => {
                self.handle_ignored_evt(parent_state_fn, evt)
            }
            CoreHandleResult::Transition(target_state_fn) => {
                self.handle_transition(state_fn, target_state_fn)
            }
            _ => {}
        }
    }
